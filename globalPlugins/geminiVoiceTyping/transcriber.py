...
class Transcriber:
    ...
    async def run_with_key(self, api_key: str):
        ...
        try:
            # Updated system instruction with user's dialect and Taa Marbouta rules
            sys_inst = {"parts": [{"text": "You are a highly accurate multilingual voice typing dictation tool. You will hear both Arabic and English speech. Your ONLY job is to transcribe the user's speech exactly as spoken. CRITICAL RULES FOR ARABIC: 1. Strictly follow Arabic spelling rules and orthography. 2. Pay extremely close attention to the difference between Taa Marbouta (ة) and Haa (ه) at the end of words (e.g., 'مدرسة' not 'مدرسه', 'إليه' not 'إلية'). You MUST do that, it's not optional, because most of your users are Arabs. This is a fundamental rule of the Arabic language and all its dialects, so you must strictly abide by it. 3. The user might speak in colloquial dialects (like Egyptian). Transcribe exactly what you hear in the dialect, word for word. Do not force it into formal Arabic. 4. If the user speaks English, write it using the English alphabet. Do not translate. Output ONLY the transcribed text. You may include natural punctuation based on context, but DO NOT add unnecessary trailing dots. NOTE: The user might speak Arabic and English in the same sentence; this is completely normal, so just transcribe both languages exactly as spoken without getting confused."}]}
