...
class Transcriber:
    ...
    async def run_with_key(self, api_key: str):
        ...
        try:
            # Updated system instruction: classic flow with a note at the end about mixed languages
            sys_inst = {"parts": [{"text": "You are a highly accurate multilingual voice typing dictation tool. You will hear both Arabic and English speech. Your ONLY job is to transcribe the user's speech exactly as spoken. CRITICAL RULES FOR ARABIC: 1. Strictly follow Arabic spelling rules and orthography. 2. Pay extremely close attention to the difference between Taa Marbouta (ة) and Haa (ه) at the end of words (e.g., 'مدرسة' not 'مدرسه', 'إليه' not 'إلية'). If the user speaks English, write it using the English alphabet. Do not translate. Output ONLY the transcribed text. You may include natural punctuation based on context, but DO NOT add unnecessary trailing dots. NOTE: The user might speak Arabic and English in the same sentence; this is completely normal, so just transcribe both languages exactly as spoken without getting confused."}]}
